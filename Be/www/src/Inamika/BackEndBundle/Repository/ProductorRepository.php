 ...
 */
class ProductorRepository extends \Doctrine\ORM\EntityRepository
{
        ...
        if($query){
            ...
            if(count($words)>1){
                foreach ($words as $key => $word) {
                    $queryString=array();
                    $queryString[]="CONCAT(e.name,e.email,COALESCE(e.observation,''),COALESCE(e.phone,'')) LIKE :word".$key;
                    $qb->setParameter('word'.$key,"%".$word."%");
                    $qb->andWhere(join(' AND ',$queryString));
                }
            }else{
                $qb->andWhere("CONCAT(e.name,e.email,COALESCE(e.observation,''),COALESCE(e.phone,'')) LIKE :query")->setParameter('query',"%".$query."%");
            }
        }
        return $qb;
    }
