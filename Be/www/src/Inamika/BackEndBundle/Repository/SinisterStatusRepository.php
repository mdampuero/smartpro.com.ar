...
/**
 * SinisterStatusRepository
 *
 * This class was generated by the Doctrine ORM. Add your own custom
 * repository methods below.
 */
class SinisterStatusRepository extends \Doctrine\ORM\EntityRepository
{
        ...
        if($query){
            ...
            if(count($words)>1){
                foreach ($words as $key => $word) {
                    $queryString=array();
                    $queryString[]="CONCAT(e.name) LIKE :word".$key;
                    $qb->setParameter('word'.$key,"%".$word."%");
                    $qb->andWhere(join(' AND ',$queryString));
                }
            }else{
                $qb->andWhere("CONCAT(e.name) LIKE :query")->setParameter('query',"%".$query."%");
            }
        }
        return $qb;
    }
